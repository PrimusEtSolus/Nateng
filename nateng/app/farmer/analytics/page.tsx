...
import { mockWholesaleOrders, mockCrops } from "@/lib/mock-data"
import { TrendingUp, DollarSign, Package, Users, ArrowUpRight } from "lucide-react"

export default function FarmerAnalyticsPage() {
  const completedOrders = mockWholesaleOrders.filter((o) => o.status === "completed")
  const totalRevenue = completedOrders.reduce((sum, o) => sum + o.total, 0)
  const totalOrders = mockWholesaleOrders.length
  const avgOrderValue = totalOrders > 0 ? Math.round(totalRevenue / completedOrders.length) : 0

  const cropSales = mockCrops
    .map((crop) => {
      const orders = mockWholesaleOrders.filter((o) => o.cropId === crop.id)
      const revenue = orders.reduce((sum, o) => sum + o.total, 0)
      const quantity = orders.reduce((sum, o) => sum + o.quantity, 0)
      return {
        ...crop,
