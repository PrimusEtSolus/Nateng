    ...
    <aside className="w-[280px] h-screen bg-white border-r border-border flex flex-col fixed left-0 top-0 shadow-sm">
      ...
      <div className="p-4 mx-4 mt-4 bg-gradient-to-br from-buyer-bg to-orange-50 rounded-xl">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-buyer rounded-full flex items-center justify-center text-white font-semibold overflow-hidden">
...
<div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate">
              {[user?.city, user?.province].filter(Boolean).join(", ") || user?.email || "Baguio, Benguet"}
            </p>
          </div>
        </div>
      </div>
