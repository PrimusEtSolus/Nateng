...
const navItems = [
  { name: "Dashboard", href: "/bulkBuyer/dashboard", icon: LayoutDashboard },
  { name: "Buy Wholesale", href: "/bulkBuyer/browse", icon: ShoppingBag },
  { name: "My Inventory", href: "/bulkBuyer/inventory", icon: Store },
  { name: "Orders", href: "/bulkBuyer/orders", icon: Package },
  { name: "Sales", href: "/bulkBuyer/sales", icon: TrendingUp },
]

export function BulkBuyerSidebar() {
  ...
  return (
    <aside className="w-[280px] h-screen bg-white border-r border-border flex flex-col fixed left-0 top-0 shadow-sm">
      ...
      <div className="p-5 border-b border-border">
        <Link href="/bulkBuyer/dashboard" className="block">
          <Logo size="md" variant="bulkBuyer" />
        </Link>
        <div className="mt-3">
      ...
      <div className="p-4 border-t border-border space-y-1">
        <Link
          href="/bulkBuyer/settings"
          className={cn(
            "flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200",
            pathname === "/bulkBuyer/settings"
